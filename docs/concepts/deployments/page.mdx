...
Temps clones your Git repository at the specified commit. For monorepos, the build runs from the configured root directory. If **Pull only the root directory** is enabled on Git settings, the clone fetches that subdirectory only.
