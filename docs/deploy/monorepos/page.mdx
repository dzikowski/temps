...
If the app does not need files outside that subdirectory (no workspace lockfile, no shared packages), enable **Pull only the root directory** on Git settings. Deploy then sparse-checkouts that path instead of fetching the rest of the tree. Leave it off when the build still needs the repository root — Turborepo, pnpm workspaces, and Dockerfiles that `COPY` sibling packages all do.
