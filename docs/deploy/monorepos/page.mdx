...
If the app does not need files outside that subdirectory (no workspace lockfile, no shared packages), enable **Pull only the root directory** on Git settings. Deploy then sparse-checkouts that path in cone mode, which still materializes repository-root files and skips other subtrees. Leave it off when the build still needs the rest of the tree — Turborepo, pnpm workspaces, and Dockerfiles that `COPY` sibling packages all do.
